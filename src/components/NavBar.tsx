import { Setter, type Component, type Accessor } from 'solid-js'


const NavBar: Component<{start: (fn: () => void, cb?: () => void) => void, setTab: Setter<number>, tab?: Accessor<number>}> = (props) => {
  const updateTab = (index: number) => () => {
    props.start(() => props.setTab(index))
  }
  const isCurrentTab = (index: number) => props.tab ? props.tab() === index : false;

  return (
    <div class="btm-nav bg-primary-content text-primary">
      <button class={`bot-nav-bar ${isCurrentTab(0) ? 'active' : ''}`} onClick={updateTab(0)}>
      ...
      <button class={`bot-nav-bar ${isCurrentTab(1) ? 'active' : ''}`} onClick={updateTab(1)}>
        <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
      ...
      <button class={`bot-nav-bar ${isCurrentTab(2) ? 'active' : ''}`} onClick={updateTab(2)}>
        <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
      ...
      <button class={`bot-nav-bar ${isCurrentTab(3) ? 'active' : ''}`} onClick={updateTab(3)}>
